...
async function simplefetch(path: string, init: RequestInit) {
  ...
  return request.then(async (response) => {
    ...
    if (response.status === 401) {
      throw new ResponseStatusError("Usuário não esta autenticado.", response.status, response.statusText);
    }
  
    const json = await response.json() as Record<string, any>
    if (!json?.message) {
      throw new ResponseStatusError("Ocorreu um erro ao fazer uma requisição", response.status, response.statusText)
    }
