...
class TaskService(
    ...
    fun findById(id: UUID): Task {
        return tasks.findByIdAndAuthorId(id, getUserId()).orElseThrow {
            ResponseStatusException(HttpStatus.NOT_FOUND, "A tarefa solicitada não foi encontrada.")
        }
    }
    ...
    fun updateTask(id: UUID, dto: TaskDTO) {
        ...
        if (task.endsIn.isBefore(OffsetDateTime.now()) || task.completed) {
            throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Não é possivel atualizar uma tarefa que já foi concluida.")
        }
